...
appConf = {
    "OAUTH2_CLIENT_ID": "696489702018-6ufophl5cone35ehp28umug20sn8mp0k.apps.googleusercontent.com",  # Google OAuth client ID
...
def home():
    ...
    return render_template("home_page.html", session=session.get("user"),
                           pretty=json.dumps(session.get("user"), indent=4))  # Render home_page.html template with user session data

# Google login route
@app.route("/google-login")  # Define route for Google login
def googleLogin():
    """Redirect to the Google OAuth authorization endpoint."""
    return oauth.myApp.authorize_redirect(redirect_uri=url_for("googleCallback", _external=True))  # Redirect to Google OAuth authorization endpoint
...
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=5000, debug=True)  # Run Flask application
